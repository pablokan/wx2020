...
#-----------------------------Metodo de busqueda de turnos -----------------------------------------------------#
    ...
    def clientesdehoy(self,datos):
        ...
        for tup in valores:
            ...
            turno =  ' '.join(tupLis) #convierto las tuplas del fetchall en strings
            listaTurno.append(turno)
        print("listaTurno:", listaTurno)
        self.lb1.SetItems(listaTurno) # agrego los agendados de fecha de hoy al ListBox
        # LA VARIABLE "valores" ES LA QUE TIENE LOS VALORES DE LO CLEINTES

        if len(listaTurno) == 0: # no va el try porque no va dar error aunque devuelva cero registros
